...
final class HomeExtensionImp(system: ActorSystem) extends HomeExtension {
  ...
  override val bap: ActorRef = HomeProcessorRegion.start().ref

  override def createHome(homeId: Int, owner: Owner, address: String, houseArea: Int, createdAt: Long): Future[Any] =
    (bap ? CreateHome(homeId, owner, address, houseArea)).mapTo[Any]
